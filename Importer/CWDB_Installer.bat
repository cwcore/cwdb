...
:install
set dbpath=../Database/Development
set mysql=.
...
for %%i in (%dbpath%\*.sql) do if %%i neq %dbpath%\*.sql) if %%i neq %dbpath%\*.sql) if %%i neq %dbpath%\*.sql) echo %%i & %mysql%\mysql -q -s -h %svr% --user=%user% --password=%pass% --port=%port% %wdb% < %%i
